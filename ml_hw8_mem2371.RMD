...
I would use an elastic net regularization algorithm. Elastic net is highly interpretable, which is necessary when conducting feature selection, as we want to be able to distinguish and understand the features that have been selected; we want to be able to communicate that the selected features are probable risk factors. Additionally, elastic net is more flexible than LASSO, and we want to run a flexible algorithm to be able to select appropriate features.
